...
const SingleBlog = () => {
  ...
  const handleSubmit = async (name, email, comment) => {
    ...
    try {
      ...
      const resComment = await axios.post(`${BASE_URL}/blogs/${id}/comments`, {
        name,
        email,
        comment,
      });
    ...
    try {
      ...
      setBlog((prev) => {
        return {
          ...
          comments: prev.comments.filter((com) => com._id !== commentId), // Filter out the comment by ID
        };
      });

...
const TourPlanForm = ({ onClose, editPlan }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const hasAdultPrice =
      adultPrice !== undefined && adultPrice !== null && adultPrice !== "";
    const hasChildPrice =
    ...
    if (
      ...
      toast.error("Both adultPrice and childPrice must be provided together.");
      setLoading(false);
    ...
    if ((hasAdultData && !hasChildData) || (!hasAdultData && hasChildData)) {
      toast.error("Both adultData and childData must be provided together.");
      setLoading(false);
  ...
  useEffect(() => {
    const fetchPlanData = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/plans/${editPlan}`);
        ...
        setTransportation(planData.transportation || { en: "", ar: "" });
        ...
        setKnowBeforeYouGo(planData.knowBeforeYouGo || [{ en: "", ar: "" }]);
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center bg-custom-yellow-light z-20">
      ...
      >
        ...
        <div>
          <div>
            <label className="font-bold">
              Stop sales <span className="text-gray-600 ml-2">(optional)</span>
            </label>
          </div>
        ...
        <div className="mt-4">
          <label htmlFor="knowBeforeYouGo" className="block mb-2 font-bold">
            Know Before You Go{" "}
            <span className="text-gray-600 ml-2">(optional)</span>
          </label>
        ...
        <div className="mt-4">
          <label htmlFor="faq" className="block mb-2 font-bold">
            FAQs <span className="text-gray-600 ml-2">(optional)</span>
          </label>
